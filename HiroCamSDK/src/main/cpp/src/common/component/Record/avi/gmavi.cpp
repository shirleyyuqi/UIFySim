...
static int avi_write_data(FILE *file, unsigned char *data, int size)
{
...
static void avi_read_data(FILE *file, unsigned char *data, int size)
{
...
*/
    if(whence==GMAVI_SEEK_GIVEN_INDEX)
    {
       ...
       // avi_file->frame_count = offset->index/sizeof(AviIndex);
    }
...
int GMAVISeekToFrame(HANDLE handle, std::map<int, GMFrameIndex> mapFrameIndexInfo, int frame, int *pOffset)
{
	GmAviFile *avi_file = (GmAviFile*) handle;

	int ret = GMSTS_OK;
	AviIndex  index;
    int dst_streamid = GM_MAKE_FOURCC('0','0','d','c');
	int streamid;

    int videoFrames = 0;
#if 0
    fseek(avi_file->file, avi_file->entry.IndexStart+totalFrames*sizeof(AviIndex), SEEK_SET);
    while( 1 )
    {
    //	printf("[AVI] :%d\n", ftell(avi_file->file));
    	if( ftell(avi_file->file) == -1 )
    	{
    		printf("[AVI] seek pos error, frame=%d\n", frame);
    		ret = GMSTS_SEEK_FAILED;
    		break;
    	}
    	...
    	if( index.dwChunkId == streamid )
    	{
    		...
    		{
    			printf("[AVI] seek total frame=%d, videoFrame=%d\n", totalFrames, videoFrames);
    			break;
    		}
    	}
    ...
    while( retryCnt++ < 100 )
    {
    	...
    	{
			avi_read_index(avi_file->file, &index);
	  // 	printf("[%d] flag:0x%x, chunkId:0x%x, streamId:0x%x\n", offset, index.dwFlags, index.dwChunkId, streamid);
			if( (index.dwFlags == 0x10) && (index.dwChunkId == streamid) ) // key frame
			{
				avi_file->data_pos = index.dwOffset-4;
				printf("[AVI] seek to dataPos:%d\n", avi_file->data_pos);
				avi_file->frame_count = totalFrames;
	...
	std::map<int, GMFrameIndex>::iterator it = mapFrameIndexInfo.find(frame);
	if (it != mapFrameIndexInfo.end())
	{
		if (it->second.bIsIFrame)
		{
			avi_file->data_pos = it->second.iDataPos;
			avi_file->frame_count = it->second.iFrameCount;
			avi_file->index_pos = it->second.iIndexPos;
			avi_file->frame_count++;
			videoFrames = it->first;
		}
		else
		{
			for (; it != mapFrameIndexInfo.begin(); it--)
			{
				if (it->second.bIsIFrame)
				{
					avi_file->data_pos = it->second.iDataPos;
	...
	while (true)
	{
		...
		//read data
		//printf("====fp:%d====\n", ftell(avi_file->file) );
		//printf("frame_count:%d, dataPos:%d\n", avi_file->frame_count, avi_file->data_pos);
		avi_get_chunk(avi_file->file, &streamid, &strdata_len);
		strdata_len += strdata_len%2;     /* WORD alignment */
		fseek(avi_file->file, strdata_len, SEEK_CUR);
		...
		if (streamid == dst_streamid)
		{
			...
			if ((index.dwFlags & 0x10) == 0x10)
			{
				...
				{
					//key帧已找到
					seekDataSize += (8+strdata_len);
					if(junk_size)
						seekDataSize += (8+junk_size);

					avi_file->data_pos -= seekDataSize;
					avi_file->index_pos -= (seekFrame - 1) * sizeof(AviIndex);
					avi_file->frame_count -= seekFrame;
					break;
				}
			}
			else
...
#if 0
	*pOffset = (videoFrames - seekPFrame);
#else
	*pOffset = videoFrames;
#endif
#endif
...
int GMAVISetStreamHeader(HANDLE handle, AviStreamHeader *avi_stream_header, GmAviStreamFormat *avi_stream_format, int *out_streamid)
{
  ...
  CRITICAL_SECTION_UNLOCK(avi_file->CriticalSection);
    ...
    int fccType = 0;
    memcpy( &fccType, avi_stream_header->fccType, sizeof(int) );
...
int GMAVISetStreamDataAndIndex(HANDLE handle, int streamid, unsigned char *data, int length, int intra,
                                      unsigned char *extra_data, int extra_length,int totalsize)
{
    GmAviFile *avi_file = (GmAviFile*) handle;
    AviIndex   index;
    ...
    if( avi_file->movi_len == 0 ) // auto size mode
    {
    	if( avi_file->data_pos > totalsize )
    	{
    		 return GMSTS_DATA_FULL;
    	}
    }
	...
	avi_put_chunk(avi_file->file, streamid, length);

	int wRet;
    wRet = avi_write_data(avi_file->file, data, length);
	if (wRet != 1)
	{
        wRet = HR_FILE_GETERR();
		if (wRet == 112)
			return GMSTS_DISK_FULL;
	}
...
int GMAVIGetAviMainHeader(HANDLE handle, AviMainHeader *avi_main_header)
{
    ...
    int fccType = 0;
    memcpy( &fccType, avi_main_header->fcc, sizeof(int) );
    if(fccType!=GM_MAKE_FOURCC('a','v','i','h'))
...
int GMAVIGetStreamHeader(HANDLE handle, int num, AviStreamHeader *avi_stream_header, GmAviStreamFormat *avi_stream_format, int *out_streamid)
{
    ...
    int fccType = 0;
    memcpy( &fccType, avi_stream_header->fcc, sizeof(int) );
    if(fccType == GM_MAKE_FOURCC('v','i','d','s'))
...
int GMAVIGetStreamDataAndIndex(HANDLE handle, int *streamid, unsigned char *data,
    ...
    int frame_no, int reverse,int *pos, int* newPos)
{
    ...
    *length = index.dwSize;
    ...
    {
        fseek(avi_file->file, avi_file->entry.IndexStart+avi_file->frame_count*sizeof(AviIndex), SEEK_SET);
     //   printf("=====IndexStart=%x, frameCount:%d======\n",avi_file->entry.IndexStart,avi_file->frame_count );
        avi_read_index(avi_file->file, &index);
        if ((index.dwFlags & 0x10) == 0x10)
        	*intra = 1;//I frame
        else
...
int GMAVIConditionIndexSearch(HANDLE handle, GMSearchIndex *out_value, int mode, int value1, int value2)
{
    ...
    else if( mode == GMAVI_SEARCH_GivenIndex )
    {
        int streamid = value1;
        int offset = value2;
        ...
        while( retryCnt++ < 100 )
        {
        	fseek(avi_file->file, avi_file->entry.IndexStart+offset*sizeof(AviIndex), SEEK_SET);
        	if( ftell(avi_file->file) == -1 )
        	{
        		ret = GMSTS_SEEK_FAILED;
        		break;
       ...
       // 	printf("[%d] flag:0x%x, chunkId:0x%x, streamId:0x%x\n", offset, index.dwFlags, index.dwChunkId, streamid);
        	if( (index.dwFlags == 0x10) && (index.dwChunkId == streamid) ) // key frame
        	{
                out_value->offset = index.dwOffset-4;;
                out_value->index = offset*sizeof(AviIndex);
...
int GMAVISearchFrameIndexInfo(HANDLE handle, std::map<int, GMFrameIndex> &mapFrameIndex)
{
	GmAviFile *avi_file = (GmAviFile*) handle;
	int ret;
	AviIndex  index;
	int streamid;
	GMFrameIndex FrameIndex = {0};
	bool bIsStart = true;
	int strdata_len, junk_tag, junk_size;
	avi_file->data_pos = 0;
	mapFrameIndex.clear();
	...
	while (true)
	{
		if( avi_file->data_pos>=(avi_file->movi_len-4) )
		{
			...
			if (bIsStart == false)
			{
				mapFrameIndex[videoFrameIndex - 1] = FrameIndex;
			}
			avi_file->data_pos = 0;
			break;
		}
		...
		fseek(avi_file->file, avi_file->entry.IndexStart+avi_file->frame_count*sizeof(AviIndex), SEEK_SET);
		avi_read_index(avi_file->file, &index);
		if (streamid == GM_MAKE_FOURCC('0','0','d','c'))		//视频帧
		{
			if ((index.dwFlags & 0x10) == 0x10)		//关键帧
			{
				FrameIndex.bIsIFrame = true;
			}
			else
			{
				FrameIndex.bIsIFrame = false;
			}

			if (bIsStart)
			{
				FrameIndex.iFrameIndex = videoFrameIndex;
				FrameIndex.iDataPos = avi_file->data_pos;
				FrameIndex.iFrameCount = avi_file->frame_count;
				FrameIndex.iIndexPos = avi_file->frame_count*sizeof(AviIndex);
				bIsStart = false;
			}
			else
			{
...
int GMAVIFillAviMainHeaderValues(AviMainHeader *header, int width, int height, int framerate, int bitrate, int framecount)
{
    ...
    header->dwStreams = 2;                  //<==need update
                                ...
                                int width, int height, int framerate, int bitrate, int framecount)
{
   ...
   // *((unsigned int*)header->fccType) = fcc_type;
    ...
    header->rcFrame.right = (short)width;
    header->rcFrame.bottom = (short)height;
...
int GMAVIFillAudioStreamHeaderValues(AviStreamHeader *header, GmAviStreamFormat *format, int type,
                                int channels, int sample_rate, int bitrate, int framecount)
{
    int fcc_type=0;
    if(!header)
        return GMSTS_INVALID_INPUT;
  ...
  //  *((unsigned int*)header->fccHandler) = 0/*type*/;
    memset( header->fccHandler, 0x00, sizeof(int) );
    header->dwFlags = 0x0;
    header->wPriority = 0;
    header->wLanguage = 0;
    header->dwInitialFrames = 0;
    header->dwScale = 1;
    header->dwRate = sample_rate*header->dwScale;
    header->dwStart = 0;
    header->dwLength = framecount;          //<==need update
  //  header->dwSuggestedBufferSize = 8000*2;
    header->dwQuality = -1;
    header->dwSampleSize = 2; //temp

    memset(format, 0x0, sizeof(GmAviStreamFormat));
    format->audio_format.wFormatTag = 1;
    format->audio_format.nChannels = (short)channels;
    format->audio_format.nSamplesPerSec = sample_rate;
    format->audio_format.nAvgBytesPerSec = sample_rate*2;
    format->audio_format.nBlockAlign = 2;
    format->audio_format.wBitsPerSample = 16; // temp
    format->audio_format.cbSize = 0;
