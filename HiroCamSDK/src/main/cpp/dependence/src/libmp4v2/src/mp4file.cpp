...
{
    if( IsWriteMode() ) {
        SetIntegerProperty( "moov.mvhd.modificationTime", 0/*MP4GetAbsTimestamp()*/ );
        FinishWrite(options);
    }
...
#if 0
    if (ATOMID("soun") == ATOMID(normType)) {
        MP4StringProperty* pStringPropertyName = NULL;
        (void)pTrakAtom->FindProperty("trak.mdia.hdlr.name",
                                      (MP4Property**)&pStringPropertyName);
        ASSERT(pStringProperty);
        pStringPropertyName->SetFixedLength(13);
        pStringPropertyName->SetValue("SoundHandler");

    }else if(ATOMID("vide") == ATOMID(normType)){
        MP4StringProperty* pStringPropertyName = NULL;
        (void)pTrakAtom->FindProperty("trak.mdia.hdlr.name",
                                      (MP4Property**)&pStringPropertyName);
        ASSERT(pStringProperty);
        pStringPropertyName->SetFixedLength(13);
        pStringPropertyName->SetValue("VideoHandler");
    }
#endif
